...
`cmd/nanci` contains the CLI entrypoint. Core application logic lives in `internal/app`, with CLI adapters in `internal/cli`, sync orchestration in `internal/sync`, and persistence in `internal/store` plus `internal/store/migrations_v2`. Domain-specific code is split across `internal/adn`, `internal/nfse`, `internal/nfe`, `internal/sefaz`, `internal/report`, `internal/files`, and `internal/foundation`. The desktop app is under `internal/desktop`: Go/Wails backend files at the module root and the Vue 3 frontend in `internal/desktop/frontend/src`.
...
Follow standard Go formatting: tabs, `gofmt`, and `goimports` with the local prefix `github.com/vasfvitor/nanci`. Keep Go packages lowercase and focused; exported identifiers use `CamelCase`, internal helpers use `camelCase`. Vue/TypeScript files use the existing ESLint + Prettier setup. Match current naming patterns such as `DocumentsPage.vue`, `MainLayout.vue`, and `PasswordPromptDialog.vue`. Brazilian fiscal domain terms from the tax authority vocabulary stay in Portuguese across all layers (Go, DTO, frontend types, DB columns): `ChaveAcesso`/`ChavesAcesso` (chave de acesso da NFS-e ou NF-e), `CNPJ`, `Competence` (competência), `tomada`/`prestada`/`intermediario` (document direction), `chave_acesso` (DB column). Do not rename these to English. The rule covers identifiers too: manifestação is `Manifestacao*` in Go, DTO and TypeScript names (`ManifestacaoRecord`, `TipoManifestacao`) and `manifestacao`/`manifestacoes` in SQL (`nfe_manifestacoes`). English verbs around a Portuguese noun are fine, as in `RegisterManifestacao` or `ListNFePendingManifestacoes`.
