...
func (i encryptedContentInfo) Data() ([]byte, error) {
	if i.EncryptedContent.Class != 2 || i.EncryptedContent.Tag != 0 {
		return nil, errors.New("pkcs12: invalid encrypted content tag")
	}
	if !i.EncryptedContent.IsCompound {
		return i.EncryptedContent.Bytes, nil
	}

	var encrypted []byte
	remaining := i.EncryptedContent.Bytes
	for len(remaining) > 0 {
		var chunk asn1.RawValue
		var err error
		remaining, err = asn1.Unmarshal(remaining, &chunk)
		if err != nil {
			return nil, fmt.Errorf("pkcs12: invalid constructed encrypted content: %w", err)
		}
		if chunk.Class != 0 || chunk.Tag != 4 || chunk.IsCompound {
			return nil, errors.New("pkcs12: constructed encrypted content contains a non-OCTET STRING chunk")
		}
		encrypted = append(encrypted, chunk.Bytes...)
	}
	return encrypted, nil
}

func (i *encryptedContentInfo) SetData(data []byte) {
	i.EncryptedContent = asn1.RawValue{
		Class: 2,
		Tag:   0,
		Bytes: data,
	}
...
func (i encryptedPrivateKeyInfo) Data() ([]byte, error) {
	return i.EncryptedData, nil
}
