...
type ResetCounts struct {
	...
	ExportMarks       int
	ManifestacoesKept int // nfe_manifestacoes stay as the audit trail of what was sent
}

// Statuses of an outbound manifestação, stored in nfe_manifestacoes.status.
const (
	ManifestacaoStatusRegistrada   = "registrada"
	ManifestacaoStatusJaRegistrada = "ja_registrada"
	ManifestacaoStatusRejeitada    = "rejeitada"
	ManifestacaoStatusErro         = "erro" // the lote got no SEFAZ answer
)
...
type ManifestacaoRecord struct {
	...
	Status        string // one of the ManifestacaoStatus* constants
	CStat         string
	XMotivo       string
	Protocolo     string
