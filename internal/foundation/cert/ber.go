...
// normalizePKCS12BER converts the BER forms used by PKCS#12 into DER. Primitive
// contents are preserved; constructed OCTET STRING chunks are concatenated.
func normalizePKCS12BER(ber []byte) ([]byte, bool, error) {
	if len(ber) == 0 {
		return nil, false, errors.New("ber2der: input BER is empty")
	}
	...
	if offset != len(ber) {
		return nil, false, errors.New("ber2der: trailing data after ASN.1 object")
	}

	root, ok := obj.(asn1Structured)
	if !ok || len(root.tagBytes) != 1 || root.tagBytes[0] != 0x30 {
		return nil, false, errors.New("ber2der: PKCS#12 root must be a SEQUENCE")
	}
	...
	der := out.Bytes()
	changed := parser.changed || !bytes.Equal(ber, der)
	return der, changed, nil
}

func ber2der(ber []byte) ([]byte, error) {
	der, _, err := normalizePKCS12BER(ber)
	return der, err
}
