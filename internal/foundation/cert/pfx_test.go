...
func TestLoadPKCS12_ValidMockCert_BER(t *testing.T) {
	mockPfxPath := filepath.Join("testdata", "cert_a1_mock_70860312000150.pfx")
	pfxData, err := os.ReadFile(mockPfxPath) //nolint:gosec // intentional: test
	if err != nil {
	...
	if err := os.WriteFile(berFile, berData, 0o600); err != nil { //nolint:gosec // intentional: test
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(tamperedFile, tampered, 0o600); err != nil { //nolint:gosec // intentional: test
		t.Fatal(err)
	}
